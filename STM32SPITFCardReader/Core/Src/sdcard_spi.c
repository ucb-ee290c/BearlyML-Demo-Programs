...
void SDCARD_init(SDCard *card, SPI_HandleTypeDef *hspi, GPIO_TypeDef* cs_gpio_block, uint16_t cs_gpio_pin) {
  card->hspi = hspi;
  card->cs_gpio_block = cs_gpio_block;
  card->cs_gpio_pin = cs_gpio_pin;
}
...
uint8_t SDCARD_transmitCommand(SDCard *card, uint8_t command, uint32_t data, uint8_t crc) {
  ...
  HAL_SPI_TransmitReceive(card->hspi, tx_buffer, rx_buffer, 6, 100);
  ...
  if (command == SDCARD_STOP_TRANSMISSION) {
    HAL_SPI_Transmit(card->hspi, tx_buffer, 1, 100);
  }

  while (n_retries > 0) {
    // wait for a valid response, transmitting 0xFF to clock the slave
    HAL_SPI_TransmitReceive(card->hspi, tx_buffer, rx_buffer, 1, 100);
    n_retries -= 1;
...
uint32_t SDCARD_receiveData(SDCard *card) {
  ...
  HAL_SPI_TransmitReceive(card->hspi, tx_buffer, rx_buffer, 4, 100);
...
SDCard_Type SDCARD_initializeCard(SDCard *card, uint32_t timeout) {
  ...
  for (uint16_t i=0; i<20; i+=1) {
    HAL_SPI_Transmit(card->hspi, &buf, 1, 100);
  }
...
void SDCARD_readSector(SDCard *card, uint32_t sector_address, uint8_t *buffer) {
  ...
  while (resp != 0xFEU) {
    HAL_SPI_TransmitReceive(card->hspi, &tx_buffer, &resp, 1, 100);
  }
