...
static inline void SDCARD_setCSHigh(SDCard *card) {
  HAL_GPIO_WritePin(card->cs_gpio_block, card->cs_gpio_pin, GPIO_PIN_SET);
}

static inline void SDCARD_setCSLow(SDCard *card) {
  HAL_GPIO_WritePin(card->cs_gpio_block, card->cs_gpio_pin, GPIO_PIN_RESET);
}

void SDCARD_init(SDCard *card, SPI_HandleTypeDef *hspi, GPIO_TypeDef* cs_gpio, uint16_t cs_gpio_pin);
